...
class TestPageSizes:
    def test_a_default_page_larger_than_the_maximum_is_refused(self) -> None:
        # Every request that omits a limit would get a page no caller is allowed to ask for.
        with pytest.raises(ValueError, match="recall_default_limit must not exceed"):
            build(recall_default_limit=101, recall_max_limit=100)

    def test_a_default_equal_to_the_maximum_is_allowed(self) -> None:
        assert build(recall_default_limit=50, recall_max_limit=50).recall_default_limit == 50
